package vn.badatdp69.ungdungtracnghiem.common;
...
public class Common {
    ...
    //Hiển thị dialog điểm chi tiết cho lần thi
    public static void showScoreDialog(Context context, ArrayList<Score> scores, int position){
        final Dialog dialog = new Dialog(context);
        if (dialog.getWindow() != null) {
            dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        ...
        TextView tvSDate = dialog.findViewById(R.id.tvSDate);
        tvSDate.setText(String.format("Ngày thi: %s (%s)",
                Common.unixTimeToDate(scores.get(position).getTime()),
                Common.unixTimeToTime(scores.get(position).getTime())));
        TextView tvSExam = dialog.findViewById(R.id.tvSExam);
        tvSExam.setText(String.format(Locale.getDefault(), "Đề số: %d", scores.get(position).getExam()));
        TextView tvSTrue = dialog.findViewById(R.id.tvSTrue);
        tvSTrue.setText(String.format(Locale.getDefault(), "Đúng: %d", scores.get(position).getCorrect()));
        TextView tvSFalse = dialog.findViewById(R.id.tvSFalse);
        tvSFalse.setText(String.format(Locale.getDefault(), "Sai: %d",
                scores.get(position).getTotal() - scores.get(position).getAnswered()));
        TextView tvSUnAns = dialog.findViewById(R.id.tvSUnAns);
        tvSUnAns.setText(String.format(Locale.getDefault(), "Chưa trả lời: %d",
                scores.get(position).getTotal() - scores.get(position).getAnswered()));
        TextView tvSScore = dialog.findViewById(R.id.tvSScore);
        tvSScore.setText(String.valueOf(scores.get(position).getScore()));
        Button btnDLClose = dialog.findViewById(R.id.btnDLClose);
        btnDLClose.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
